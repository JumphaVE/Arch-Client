...
//		WorldRenderer worldrenderer = tessellator.getWorldRenderer();
	...
	public static void renderItemsESP(Entity entityIn, double x, double y, double z, float partialTicks) {
		...
		/* renamed from: mc */
	private static final Minecraft f2mc = Minecraft.func_71410_x();

	public static void drawShadedString(String text, int x, int y, int color) {
		String unformattedText = text.replaceAll("(?i)§[\\da-f]", "");
		Minecraft.getMinecraft().fontRendererObj.drawString(unformattedText, x + 1, y, 0);
		Minecraft.getMinecraft().fontRendererObj.drawString(unformattedText, x - 1, y, 0);
		Minecraft.getMinecraft().fontRendererObj.drawString(unformattedText, x, y + 1, 0);
		Minecraft.getMinecraft().fontRendererObj.drawString(unformattedText, x, y - 1, 0);
