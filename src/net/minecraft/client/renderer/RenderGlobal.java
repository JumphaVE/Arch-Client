...
import cn.enaium.client.mod.mods.render.SelectBlock;
    ...
    public void drawSelectionBox(EntityPlayer player, MovingObjectPosition movingObjectPositionIn, int p_72731_3_, float partialTicks) {
        if (p_72731_3_ == 0 && movingObjectPositionIn.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            ...
            if (block.getMaterial() != Material.air && this.theWorld.getWorldBorder().contains(blockpos)) {
                ...
                AxisAlignedBB axisAlignedBB = block.getSelectedBoundingBox(this.theWorld, blockpos).expand(0.0020000000949949026D, 0.0020000000949949026D, 0.0020000000949949026D).offset(-d0, -d1, -d2);

                Color color = Client.modManager.getByClass(SelectBlock.class).isEnable() ? new Color(255, 255, 255, 100) : new Color(0, 0, 0, 100);
                GlStateManager.color(color.getRed() / 255f, color.getGreen() / 255f, color.getBlue() / 255f, color.getAlpha() / 255f);
                if (Client.modManager.getByClass(SelectBlock.class).isEnable()) {
                    RenderUtil.drawCube(axisAlignedBB);
                } else {
